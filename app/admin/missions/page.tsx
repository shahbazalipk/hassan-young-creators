...
function MissionsManager() {
  ...
  async function onCreate(event: FormEvent<HTMLFormElement>) {
    ...
    setToast(json.ok ? "Daily mission saved." : json.error || "Failed");
    if (json.ok) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-3xl font-bold">Daily Missions</h2>
          <p className="text-slate-500">
            Create short, safe daily missions that help children practice creating one small step at a
            time.
          </p>
        </div>
        {csrfToken ? (
      ...
      <form onSubmit={onCreate} className="admin-card grid gap-3 p-5 md:grid-cols-2">
        ...
        <label className="text-sm font-semibold">
          Status
          <select className="admin-input mt-1" name="status" defaultValue="PUBLISHED">
      ...
      <div className="space-y-3">
        {challenges.length === 0 ? (
          <p className="admin-card p-5 text-slate-500">No missions yet.</p>
        ) : (
      ...
      <ConfirmDeleteDialog
        ...
        description={
          pendingDelete
            ? `Delete “${pendingDelete.title}”? This mission and any linked submissions will be permanently removed and cannot be recovered.`
            : ""
        }
        busy={deleting}
        onCancel={() => {
        ...
        onConfirm={async () => {
          ...
          setDeleting(false);
          if (result.ok) {
            setToast(result.message || "Mission deleted.");
